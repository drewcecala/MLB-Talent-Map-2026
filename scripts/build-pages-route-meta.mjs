...
const title = "High Schools Producing MLB Talent Since 2000 | MLB Talent Map 2026";
const description = "Map and rank U.S. high schools by distinct players who made an MLB debut from 2000 through August 24, 2026, with player-level source records.";
...
routeHtml = replace(/(<meta\s+property="og:title"\s+content=")[^"]*("\s*\/?>)/s, title, routeHtml);
...
routeHtml = replace(/(<meta\s+property="og:image:alt"\s+content=")[^"]*("\s*\/?>)/s, "Map of leading U.S. high schools by post-2000 MLB debuts", routeHtml);
routeHtml = replace(/(<meta\s+name="twitter:title"\s+content=")[^"]*("\s*\/?>)/s, title, routeHtml);
...
routeHtml = replace(/(<title>)[^<]*(<\/title>)/s, title, routeHtml);
...
for (const expected of [title, description, url, image, 'content="1200"', 'content="630"']) {
